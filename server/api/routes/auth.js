...
router.get("/login", redirectHome, (req, res, next) => {
  let encodedRedirectURL = encodeURIComponent(
    `${process.env.API_HOST}/auth/verify/`
  );

  res.redirect(
    `${process.env.CITY_WEBSITE_URL}/cas/login?service=${encodedRedirectURL}`
  );
});
...
router.get("/verify", (req, res, next) => {
  ...
  let encodedServiceURL = encodeURIComponent(
    `${process.env.API_HOST}/auth/verify/`
  );

  axios
    .get(
      `${process.env.CITY_WEBSITE_URL}/cas/serviceValidate?service=${encodedServiceURL}&ticket=${casTicket}&format=json`,
      {
        timeout: 2000
      }
    )
...
router.get("/logout", (req, res) => {
  req.session.destroy(err => {
    ...
    res.redirect(
      `${process.env.CITY_WEBSITE_URL}/cas/logout?url=${process.env.CLIENT_HOST}`
    );
  });
